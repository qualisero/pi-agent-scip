...
export class TypeScriptAdapter implements LanguageAdapter {
  ...
  async isIndexerAvailable(projectRoot: string): Promise<boolean> {
    try {
  ...
  async installIndexer(projectRoot: string, options?: InstallOptions): Promise<void> {
    if (options?.confirm) {
      const acknowledged = await options.confirm(
        'scip-typescript ships with pi-agent-scip via npm and is invoked automatically. No additional installation is required.'
      );
      if (!acknowledged) {
  ...
  async generateIndex(options: IndexOptions): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.on('error', (err) => {
        reject(
          new Error(
            `Failed to run scip-typescript: ${err.message}. ` +
  ...
  async getIndexerVersion(projectRoot: string): Promise<string> {
  ...
  private async hasTsconfig(projectRoot: string): Promise<boolean> {
    try {
