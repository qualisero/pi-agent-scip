...
export const createScipTools: CustomToolFactory = (pi) => {
  ...
  return [
    ...
    // List symbols in file
    {
      ...
      async execute(toolCallId, params) {
        ...
        if (!(await query.indexExists())) {
          return {
            content: [{ type: 'text' as const, text: 'SCIP index not found. Run scip_reindex first.' }],
            details: [],
          };
        }
        ...
        return {
          content: [{ type: 'text' as const, text: `Symbols in ${params.file}:\n\n${body}` }],
          details: symbols,
        };
      },
    },
    ...
    // Search symbols by name
    {
      ...
      async execute(toolCallId, params) {
        ...
        return {
          content: [{ type: 'text' as const, text: `Found ${results.length} symbol(s) matching '${params.query}':\n\n${body}` }],
          details: limited,
        };
      },
    },
    ...
    // Reindex tool
    {
      ...
      async execute(toolCallId, params, onUpdate, ctx, signal) {
        ...
        try {
          ...
          return {
            content: [{ type: 'text' as const, text: 'Reindex complete.' }],
            details: [],
          };
        } catch (error) {
          logger.log({
...
function renderTree(nodes: CodeTreeNode[], maxDepth: number): string {
  const lines: string[] = [];

  const visit = (node: CodeTreeNode, depth: number) => {
